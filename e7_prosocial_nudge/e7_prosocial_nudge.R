...
colnames(d_subset) <- c('forced_cond', 'choice')

d_subset <- as.data.frame(d_subset, stringsAsFactors=FALSE)

#extract the good d from the middle part of the raw d
for(i in 1:dim(d_raw)[1]) {
  curr <- d_raw[i,21:22][!is.na(d_raw[i,21:22])] #for a given row, get only the non NA values
...
d_raw <- cbind(d_subset, d_raw[,23:30])
d_raw$ss <- 1:dim(d_raw)[1]

d_raw$forced_cond_num <- ifelse(d_raw$forced_cond=="two",1,2)
...
colnames(choice_mat) <- c('cond', 'choice', 'proportion')
choice_mat <- as.data.frame(choice_mat, stringsAsFactors=FALSE)

counter <- 1
  for(j in 1:2) {
    for(k in 1:3) {
      choice_mat[counter,] <- c(j, k, length(d$choice[d$forced_cond_num==j & d$choice == k])/
                                  length(d$choice[d$forced_cond_num==j])*100)
      counter <- counter + 1
    }
  }

forced_labels <- c("Two", "Three")
choice_labels <- c("Proximity Rule", "No Discrimination", "Minimize Harm")
...
p1 <- BarPlotter(choice_mat, choice_mat$cond, 'Number of Choices', choice_mat$proportion, 'Proportion', choice_mat$choice, forced_labels, choice_labels, 'Outrage at Firm')

dev.new(width=9,height=8,noRStudioGD = TRUE)
p1

ggsave(
  "e6_prosocial_nudge.pdf",
  last_plot(),
  dpi = 500
)
